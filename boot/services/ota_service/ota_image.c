/*****************************************************************************
 * @file    ota_eeprom.c
 * @brief   OTA EEPROM A/B 双备份元数据读写实现
 *****************************************************************************/
 ...
 * ------------------------------------------------------------------------- */
static int ota_eeprom_read_copy(uint32_t addr, ota_eeprom_info_t *copy)
{
    int ret;

    ret = eeprom_read(OTA_EEPROM_DEVICE_NAME, addr, (uint8_t *)copy, sizeof(ota_eeprom_info_t));
    ...
    /* 魔数校验 */
    if (copy->magic != OTA_EE_MAGIC)
    {
        return 0;
    }

    /* 结构体大小合理性检查 */
    if (copy->struct_size != OTA_EE_STRUCT_SIZE)
    {
        return 0;
    }

    /* 结构体 CRC32 校验 */
    if (ota_eeprom_calc_struct_crc(copy) != copy->struct_crc32)
    {
        return 0;
    }
...
int ota_eeprom_load(ota_eeprom_info_t *info)
{
    ...
    {
        /* 两个副本均有效，取 seq 较大者 */
        *info = (copy_a.seq >= copy_b.seq) ? copy_a : copy_b;
        return RET_OK;
    }
    else if (a_valid)
    {
